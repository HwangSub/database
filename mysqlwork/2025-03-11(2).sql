...
-- 3-1. price에서 null값인 데이터 출력
  select * from bookInfo where price is null;  -- 2개
...
-- 5. price 가 null로 되어있는 데이터의 값으로 0을 세팅한다.
  update bookInfo set price=0 where price is null;
  select * from bookInfo;

-- 6. maker 가 null로 되어있는 데이터의 값으로 '하이미디어'로 세팅 
